...
**Performance:**
* **Ranking:** Top ~8% (Unofficial Silver Medal Zone) on Private Leaderboard.
* **Context:** The solution achieved a highly competitive score, demonstrating robust generalization despite a selection error during the final submission phase.
...
* **Constraints:** Physical consistency (e.g., `Green + Clover = GDM`).

---

## Solution Architecture

My approach relies on a **dual-stream ensemble** leveraging state-of-the-art Vision Transformers (ViT) and Mamba (SSM) for feature fusion.
...
* **Split-View Strategy:** Images are split into Left/Right halves to handle the wide aspect ratio and double the effective batch size.

### 2. Model 1: DINOv3 + Mamba Fusion
This is the primary regression model (~88% ensemble weight).
...
Used as a secondary model (~12% ensemble weight) to capture semantic features.
